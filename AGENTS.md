...
- The weather popup (`weather.py --popup`) is a fullscreen transparent **gtk-layer-shell** surface, not an xdg toplevel — position, CSS rounded corners, and "click outside to close" (caught by the surface itself) all live inside `weather.py`. Do not add a Hyprland `window_rule` for it, and do not reintroduce `focus-out-event` dismissal (with `follow_mouse = 1` that closes the popup on mere mouse leave).
